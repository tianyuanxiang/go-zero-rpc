...
> 本文档说�?plating-platform 项目的完整目录架构、各层职责关系，以及所�?HTTP 业务接口的路由、参数和业务逻辑�?
...
�?  └── desc/               # API分类定义目录
�?      ├── base.api        # 通用基础结构（分页请�?响应、ID请求等）
�?      ├── auth.api        # 认证相关接口定义（登�?刷新/登出/修改密码�?�?      ├── system/         # 系统管理接口定义
�?      �?  ├── user.api    # 用户管理（CRUD + 角色分配 + 密码重置�?�?      �?  ├── role.api    # 角色管理（CRUD + 菜单/API权限绑定�?�?      �?  ├── menu.api    # 菜单管理（CRUD + 树形查询�?�?      �?  ├── api.api     # API接口管理（CRUD + 分组查询�?�?      �?  ├── dict.api    # 字典管理（类�?+ 数据�?CRUD�?�?      �?  ├── log.api     # 日志管理（登录日�?+ 操作日志查询/清空�?�?      �?  └── file.api    # 文件上传管理（上�?删除/列表�?�?      └── plating/        # 槽液业务接口定义
�?          ├── tank.api    # 槽体配置管理（CRUD + 初始化模型状态）
�?          ├── event.api   # 生产/加药/补水事件录入查询 + 触发计算
�?          └── state.api   # 模型状态查�?趋势/化验校正/导出
├── docs/                   # 项目文档
�?  ├── casbin_guide.md     # Casbin权限系统使用指南
�?  └── business_api_interfaces.md  # 业务接口说明文档（本文档�?├── etc/                    # 配置文件目录
�?  ├── plating.yaml        # 应用主配置文件（端口、数据库、JWT密钥等）
�?  └── rbac_model.conf     # Casbin RBAC模型配置（策略规则定义）
├── internal/               # 私有业务代码（不对外暴露�?�?  ├── config/             # 配置结构定义
...
�?  �?  ├── system/         # 系统管理Handler（user/role/menu/api/dict/log/file�?�?  �?  └── plating/        # 槽液业务Handler（tank/event/state�?�?  ├── logic/              # 业务逻辑层（核心层，所有业务规则在此实现）
�?  �?  ├── auth/           # 认证业务逻辑（JWT生成、密码验证、登录日志写入）
�?  �?  ├── system/         # 系统管理业务逻辑（RBAC管理、字典缓存等�?�?  �?  └── plating/        # 槽液业务逻辑
�?  �?      ├── tankconfiglogic.go      # 槽体配置CRUD逻辑 + 初始化模型状�?�?  �?      ├── modelstatelogic.go      # 模型状态查�?化验校正/导出逻辑
�?  �?      └── eventlogic.go           # 生产/加药/补水事件录入 + 触发计算逻辑
�?  ├── middleware/         # HTTP中间�?�?  �?  ├── authmiddleware.go    # JWT认证中间件（解析令牌、注入用户信息到ctx�?�?  �?  └── casbinmiddleware.go  # Casbin权限校验中间件（校验路由+方法权限�?�?  ├── model/              # 数据访问层（纯SQL，无业务逻辑�?�?  �?  ├── sysuser.go          # 系统用户模型（sys_user表）
...
�?  ├── svc/                # 服务上下文（依赖注入中心�?�?  �?  └── servicecontext.go   # ServiceContext：初始化数据库连接、Model实例、JWT工具等并注入
�?  └── types/              # 请求/响应结构体定义（DTO层）
�?      └── types.go            # 所有Handler层使用的请求/响应DTO类型（由goctl生成�?├── pkg/                    # 可被外部包导入的公共工具�?�?  ├── jwtx/               # JWT工具：生成AccessToken/RefreshToken、解析、刷新令�?�?  ├── xerr/               # 统一错误码定义与业务错误类型封装
�?  └── utils/              # 通用工具函数（时间格式化、加密等�?├── schema/                 # 数据库初始化脚本
�?  └── init.sql            # 完整建表SQL + 初始数据（角�?菜单/字典种子数据�?├── go.mod                  # Go模块定义（模块名、Go版本、依赖声明）
├── go.sum                  # 依赖校验文件（由go mod tidy自动维护�?└── main.go                 # 程序入口：加载配置、初始化ServiceContext、注册路由、启动HTTP服务
```
...
[Logic层] internal/logic/
   - 包含所有业务规则和计算逻辑
   - 通过 ServiceContext 访问 Model �?   - 处理参数校验、权限过滤、数据组�?   - 机理模型公式在此层实�?   |
   v
[Model层] internal/model/
   - 封装原生 SQL 语句
   - 通过 sqlx.SqlConn 执行数据库操�?   - 只做数据存取，不含业务规�?   |
   v
[数据库] MySQL
   - 存储所有业务数�?   - 通过 schema/init.sql 初始化表结构
```

**中间件调用位�?*：在 Handler 层之前由 go-zero 框架自动调用�?
- `AuthMiddleware`：解�?JWT，将 userId/roleCode 写入 context，令牌非法则直接返回 401�?- `CasbinMiddleware`：从 context 取出 roleCode，结合请求路径和 HTTP 方法查询 Casbin 策略，无权限则返�?403�?
**ServiceContext**（`internal/svc/servicecontext.go`）是依赖注入中心，在服务启动时初始化�?- 数据库连接（sqlx.SqlConn�?- 所�?Model 实例（TankConfigModel、ModelStateModel 等）
- JWT 工具实例（jwtx�?- Casbin Enforcer 实例
...
**业务逻辑**：将当前 AccessToken 加入 Redis 黑名单，后续请求携带�?Token 将被拦截�?
---
...
**用户列表查询参数�?* `page`、`pageSize`、`keyword`（用户名/昵称模糊）、`status`�?启用/0禁用�?
---
...
| GET | /api/system/menu/current | 获取当前登录用户有权限的菜单�?|
...
| dictType | 说明 | 常用�?|
|----------|------|--------|
| plating_workpiece_type | 工件类型 | A=平板�? B=普通件, C=复杂�?|
| plating_drug_type | 药品类型 | CrO3=铬酸�? H2SO4=硫酸 |
| sys_user_status | 用户状�?| 1=启用, 0=禁用 |
...
| DELETE | /api/system/log/login/clear | 清空所有登录日�?|
| GET | /api/system/log/oper | 分页查询操作日志 |
| DELETE | /api/system/log/oper/clear | 清空所有操作日�?|

**登录日志查询参数�?* `page`、`pageSize`、`username`、`status`�?成功/0失败）、`startTime`、`endTime`
...
## 五、槽液业务接�?�?槽体配置�?api/plating/tank�?
槽体配置是机理模型计算的基本单元，存储每个槽体的物理和化学参数�?
> 所有槽液业务接口均需�?JWT 认证 + Casbin 权限校验�?
### 5.1 创建槽体配置
...
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| tankId | string | �?| 槽体唯一标识，自定义，不可修改，�?TANK-001 |
| volumeLiters | float64 | �?| 槽液工作体积（升），从设备铭牌读�?|
| densityGPerL | float64 | �?| 镀液密度（g/L），铬镀槽约1250~1350，默�?280 |
| dCrO3GPerAh | float64 | �?| 每Ah消耗的CrO3（g），论文范围0.3~0.5，初始用0.4 |
| dCr3GPerAh | float64 | �?| 每Ah生成的Cr3+（g），论文范围0.05~0.12，初始用0.08 |
| carryoverA | float64 | �?| 平板件带液系数（g/dm²），Leiden论文Table4参考值：1.0 |
| carryoverB | float64 | �?| 普通件带液系数（g/dm²），Leiden论文Table4参考值：2.0 |
| carryoverC | float64 | �?| 复杂件带液系数（g/dm²），Leiden论文Table4参考值：6.5 |
...
**业务逻辑�?*
1. 校验 tankId 对应的槽体配置存在且已激�?2. �?tankVolume �?0 或未填，自动�?tank_config.volume_liters 读取
3. 写入 model_state 表，source = "init"
4. 若已存在初始化状态记录，视为重新初始化（写入新记录，不覆盖历史）

---

## 六、槽液业务接�?�?事件管理�?api/plating/event�?
...
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| ampHour | float64 | �?| 本批次安时数（从整流器面板读取，或由TDEngine积分计算�?|
| areaDm2 | float64 | �?| 工件总表面积（dm²�? 单件面积 × 件数，用于带液量计算 |
| workpieceType | string | �?| 工件类型：A=平板件（带液系数1.0），B=普通件�?.0），C=复杂件（6.5�?|
...
公式B（工件带液带出）�?    带液质量 = areaDm2 × carryoverCoeff（由workpieceType选择A/B/C系数�?    带液体积 = 带液质量 / densityGPerL
...
| 说明 | 仅允许删�?is_processed=0（未处理）的事件，已处理事件不可删除 |
...
**drugType 枚举值：** CrO3=铬酸酐，H2SO4=硫酸，Cr2O3=三氧化二�?
**机理模型计算（加药事件触发）�?*

```
公式A（加药）：CrO3_new = CrO3_old + massGram / volume
              Cr3不变，体积不�?              （固体粉末溶解后体积变化极小�?00L槽加500g误差<0.1%，忽略不计）
```
...
```
公式E（补水稀释）�?    volume_new   = volume_old + volumeLiter
...
## 七、槽液业务接�?�?模型状态（/api/plating/state�?
模型状态是机理模型的输出结果，是前端实时看板和历史趋势图的数据来源�?
### 7.1 获取槽体最新模型状�?
| 项目 | 说明 |
|------|------|
| 路由 | GET /api/plating/state/:tankId |
| 说明 | 返回该槽�?calc_time 最新的一条状态记录，用于实时看板展示 |
...
| 说明 | 返回时间序列数据，供前端 CrO3/Cr3+ 浓度折线图渲染，�?calc_time 升序排列 |
...
|------|------|------|
| tankId | �?| 槽体ID |
| startTime | �?| 开始时间（格式�?006-01-02 15:04:05�?|
| endTime | �?| 结束时间 |
| limit | �?| 最多返回数据点数量，默�?00，最大建�?00 |
...
**前端使用方式**：提�?`calcTime` 作为 X 轴，`crO3GPerL` �?`cr3GPerL` 分别作为两条折线数据�?
---
...
2. 不对历史数据做回溯修正（取样到出结果可能间隔数小时，回溯重算成本高且意义有限�?3. 调度器以此化验值为新起点继续推算后续事�?4. calcTime 为空时使用当前时间作为校正时间点
...
|------|----------|------|------------|
| 超级管理�?| admin | 系统内置角色 | 全部接口（model.conf中跳过Casbin检查） |
| 管理�?| manager | 平台管理人员 | 系统管理全部 + 槽液分析全部 |
| 工程�?| engineer | 工艺工程�?| 槽体配置CRUD + 参数修改 + 全部查看 |
| 操作�?| operator | 生产现场操作�?| 录入生产/加药/补水事件 + 查看状态和趋势 |
| 只读 | viewer | 管理层查看人�?| 只能查看看板、趋势和历史数据 |
| 化验�?| lab | 化验室人�?| 化验校正接口 + 初始化状�?+ 查看 |
...
根据 Leiden et al. 2020 论文及《机理模型技术文�?md》，每次处理事件时对应以下公式：

```
公式A（加药）�?  CrO3_new = CrO3_old + M(g) / V(L)
                  Cr3不变，体积不�?
公式B（带液带出）：V_new = V_old - (areaDm2 × coeff / density)
                  coeff �?workpieceType 决定：A=1.0, B=2.0, C=6.5 g/dm²
                  浓度不变（带走同浓度液体�?
...
**实现位置**：建议新�?`internal/scheduler/scheduler.go`

**调度逻辑**�?1. �?分钟轮询一次所有活跃槽体（is_active=1�?2. 对每个槽体，查询所�?is_processed=0 的事件（三种类型合并后按时间升序排列�?3. �?model_state 取最新状态作为计算起�?4. 按顺序逐事件应用上述公式，每处理一个事件写入一�?model_state 记录（source="calc"�?5. 标记已处理事件为 is_processed=1
...
```
etc目录下的配置文件
路由注册文件 handler/routes.go �?所有的handler
所有的logic文件
types文件
```
...
如果表名已经分类�?
```bash
goctl model mysql datasource -url="user:password@tcp(127.0.0.1:3306)/database" -t 'plate_*' -dir="./internal/model/plate" --style go_zero
```
...
### 3、改某个接口的前端请求参数或后端返回参数，流�?
```bash
1. 在对�?.api 文件里修�?type XxxReq / type XxxResp
2. 如果是新增接口，就在api文件中对�?service 里新增路由定�?3. 重新执行 goctl api go --api .\api\plating.api --dir . --type-group --style go_zero
注意：每次修改任�?.api 文件后，始终用总入口文件生成：
```
...
2.如果通过三个运行参数反推净化效果，按照正常的业务流程，前三个模型的输入数据是同步的吗？如果测点的采集频率不一致，那么前三个模型的结果不能同步，那�?个引擎什么时候执行呢�?3.你说多参数同时异常时需要有主次排序逻辑（比如pH异常的影响权重可能高于填料老化），这个排序规则是什么？
